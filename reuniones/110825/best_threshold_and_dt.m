...
alpha = zeros(length(thresholds), 5);
tau = zeros(length(thresholds), 5);
gamma = zeros(length(thresholds), 5);
N_ava = zeros(length(thresholds), 5);
...
c=colorbar;
c.Label.String = "Umbral (\muV)";
legend([('dt = '+string(1:5)) '\gamma=(\alpha-1)/(\tau-1)']);
hold off;